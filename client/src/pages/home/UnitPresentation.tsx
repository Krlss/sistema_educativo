    ...
    <div>
      <div className="flex flex-col items-center justify-center">
        <div className="flex gap-2">
          ...
          {asignature?.unit?.name && (
            <span className="text-2xl font-bold">- {asignature.unit.name}</span>
          )}
        </div>
        <div className="mx-auto max-w-5xl pb-20 w-full">
          {asignature?.unit.topics.map(({ id, name, image, video }, index) => {
            const haveClass = image || video
                ...
                <div className="flex md:flex-row flex-col justify-between md:items-center w-full p-5">
                  ...
                  {haveClass && (
                    <div className="flex md:flex-col flex-row gap-2 mr-5">
                      <NavLink
                        to={`/asignatura/${asignatureId}/unidad/${unitId}/tema/${id}`}>
                        <li
