...
const Base10Descomposition = (props: question) => {
  const { handleChange, value_, value, gameState } =
    useBase10Descomposition(props)
  return (
    <>
      <QuestionTitle
        title={props.title}
        ...
        {value_.map((item, index) => (
          <div className="flex flex-col items-center" key={index}>
            <input
              ...
              className={`border border-gray-400 rounded px-2 py-1 w-full text-black ${
            ...
            />
            {gameState.next && item.value !== Number(item.response) && (
