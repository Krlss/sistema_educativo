...
const CartesianQuadrant = ({ type }: { type: typeCartesian }) => {
  const { xx, xy, yx, yy } = getPosTextLine(type) as {
    xx: number
    xy: number
    yx: number
    yy: number
  ...
  return (
    <>
      {[...Array(11)].map((_, i) => {
        ...
        const number = getNumbers({ isX, i, type })
        const lineX = getPosLine({ isX, isY, i, type })
        const lineY = getPosLine({ isX, isY, i, type })
        const { x, y } = getPosText({ isX, isY, type }) as {
          x: number
          y: number
        }
        return (
          <g
            ...
            />
            <line
              x1={getPosSmallLine({ isX, isY, type, pos: 'X1' })}
              y1={getPosSmallLine({ isX, isY, type, pos: 'Y1' })}
              x2={getPosSmallLine({ isX, isY, type, pos: 'X2' })}
              y2={getPosSmallLine({ isX, isY, type, pos: 'Y2' })}
              stroke="black"
              strokeWidth="1"
            />
